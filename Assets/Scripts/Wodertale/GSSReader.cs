...
public class GSSReader : MonoBehaviour {
    ...
    public async UniTask GetFromWebAsync() {   //　<=　☆①　メソッドの戻り値、キーワードの追加、メソッド名を非同期処理のメソッドであることがわかる名称に変更します。
        ...
        for (int i = 0; i < sheetDatas.Length; i++) {
            ...
            DebugLogger.Log(request.downloadHandler.text);
    ...
    private List<string[]> ConvertToArrayListFromCSV(string text) {
        ...
        while (reader.Peek() >= 0) {  // Peek メソッドを使うと戻り値の値によりファイルの末尾まで達しているか確認できる。末尾になると -1 が戻るので、そうなるまで繰り返す
            string line = reader.ReadLine();        // 一行ずつ読み込み
            string[] elements = line.Split(',');    // 行のセルは,で区切られているので、それを分割して１文字ずつの情報が入った配列にする

            for (int i = 0; i < elements.Length; i++) {　　// 1文字ずつ取り出す
                if (elements[i] == "\"\"") {
